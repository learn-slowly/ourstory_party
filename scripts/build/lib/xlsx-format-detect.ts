...
export type FormatKind = "A" | "B" | "C" | "D" | "F";

/**
 * NEC 다운로드 xlsx 의 형식 자동 감지.
 *  - C: row[0] 한 줄에 시도/구시군 header + 후보자명 합침 (2022 대선·재보궐)
 *  - F: row[0] header / row[1] 후보자명 분리 (2017 19대 대선)
...
export function detectFormat(filePath: string): FormatKind {
  ...
  // C·F: row[0] 첫 셀이 시도 + 둘째가 구시군. 후보자 위치로 분기.
  if ((row0[0] === "시도" || row0[0] === "시도명") &&
      (row0[1] === "구시군" || row0[1] === "구시군명")) {
    // 후보자명 패턴 = "정당명\n후보자" — \n 포함
    const candidatesInRow0 = row0.filter((c) => c && c.includes("\n")).length;
    const row1 = (grid[1] ?? []).map((c) => (c ?? "").toString().trim());
    const candidatesInRow1 = row1.filter((c) => c && c.includes("\n")).length;
    if (candidatesInRow0 >= 2) return "C";
    if (candidatesInRow1 >= 2) return "F";
    return "C"; // fallback — 기존 동작 유지
  }
