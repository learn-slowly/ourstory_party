...
function parse6HoeXls(xls: Buffer, sidoName: string, sigunguOverride?: string): ParsedStationRow[] {
  ...
  for (let i = 0; i < Math.min(json.length, 5); i++) {
    ...
    if (col0 === "구시군" || col0 === "구시군명" || col0 === "구  시  군") {
      perSido = true;
      headerRowIdx = i;
      break;
    }
    if (col0 === "읍면동명" || col0 === "읍  면  동  명") {
      perSido = false;
      headerRowIdx = i;
      break;
    }
  ...
  const partyRow = json[partyRowIdx] as (string | null)[];
  const partyStartCol = perSido ? 5 : 4;
  ...
  for (let r = dataStart; r < json.length; r++) {
    ...
    if (!sigungu && !emd) continue;
    if (perSido && sigungu && sigungu !== "합계") currentSigungu = sigungu;
    else if (!perSido && sigunguOverride) currentSigungu = sigunguOverride;
    ...
    } else if (gubun === "관외사전투표" || gubun === "거소투표") {
      kind = "abs";
      emdName = emd || null;
    } else {
      emdName = emd || gubun;
...
export async function parseJiseon36(outerZipPath: string): Promise<Jiseon36Result> {
  ...
  // 3회 시도지사: zip 내 per-sigungu xls들이 CP949 encoding 이슈 — 별도 처리 필요
  // 비례 시도지사는 없음 (3회엔 시도의원비례만 있음)
  // 시도지사 per-sigungu xls는 이진파일명 매핑 불가 — skip & warn
  warnings.push("3회 시도지사(2002-local-governor): CP949 파일명 이슈로 미파싱");
  ...
  for (const zipEntry of zipEntries6) {
    ...
    // extract sido from filename: "...(경남).zip" or "...(경남).zip"
    let sidoName = "";
    ...
    for (const ie of innerZip.getEntries()) {
      ...
      // determine election from folder
      const folder = iname.split("/")[0];
      const electionId = getElectionId6HoeFolder(folder);
